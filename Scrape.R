...
pbp.full$is.Rebound <- c(0, 1*(pbp.full$Event[2:nrow(pbp.full)] %in% c("GOAL", "SHOT", "MISS", "BLOCK") & pbp.full$Event[1:(nrow(pbp.full) - 1)] %in% c("GOAL", "SHOT", "MISS", "BLOCK") & pbp.full$Period[1:(nrow(pbp.full) - 1)] == pbp.full$Period[2:nrow(pbp.full)] &
                                 (as.numeric(as.character(pbp.full$Seconds[2:nrow(pbp.full)])) - as.numeric(as.character(pbp.full$Seconds[1:(nrow(pbp.full) - 1)]))) <= 2))
pbp.full$is.Rush <- c(0, 1*(pbp.full$Event[2:nrow(pbp.full)] %in% c("GOAL", "SHOT", "MISS", "BLOCK") & pbp.full$Period[1:(nrow(pbp.full) - 1)] == pbp.full$Period[2:nrow(pbp.full)] &
                              (as.numeric(as.character(pbp.full$Seconds[2:nrow(pbp.full)])) - as.numeric(as.character(pbp.full$Seconds[1:(nrow(pbp.full) - 1)]))) <= 4 & 
                              {pbp.full$Event[1:(nrow(pbp.full) - 1)] %in% c("GIVE", "TAKE") | {{pbp.full$ev.team[1:(nrow(pbp.full) - 1)] == pbp.full$ev.team[2:nrow(pbp.full)] & pbp.full$ev.zone[1:(nrow(pbp.full) - 1)] == "Def"} |
                              {pbp.full$ev.team[1:(nrow(pbp.full) - 1)] != pbp.full$ev.team[2:nrow(pbp.full)] & pbp.full$ev.zone[1:(nrow(pbp.full) - 1)] == "Off"}}}))
...
sum1p.home <- function(x) {
  summarise(x,
            ...
            SOA = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("SHOT", "MISS", "GOAL") & as.character(p1) == as.character(Player)),
            SOG = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("GOAL") & as.character(p1) == as.character(Player)),
            OTF = sum(as.character(Event) %in% c("ON") & as.character(p1) == as.character(Player) & FOS < 1),
            DIST = sum(na.omit(Distance*(as.character(Event) %in% c("SHOT", "GOAL", "MISS") & as.character(p1) == as.character(Player))))) %>% data.frame()
}
...
sum2p.home <- function(x) {
  summarise(x, P3 = "X",
            ...
            P3.G = NA, P3.A1 = NA, P3.A2 = NA) %>% data.frame()
}
...
goalie.bygame <- rbind_list(group_by(pbp.full, Season, Date, Game.ID, Home.Goalie, Score.Cat, Strength.State, Season.Type) %>% rename(Player = Home.Goalie) %>%
                            summarise(Venue = "Home", Team = first(Home.Team), TOI = round(sum(as.numeric(as.character(Event.Length)))/60, 2),
                                      ...
                                      SOA = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("SHOT", "MISS", "GOAL") & as.character(ev.team) == Away.Team),
                                      SOG = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("GOAL") & as.character(ev.team) == Away.Team),
                                      iPENT = 2*sum(as.character(Event) %in% c("PENL") & as.character(p1) == as.character(Player) & grepl("4 min", as.character(Detail)) == T) + sum(as.character(Event) %in% c("PENL") & as.character(p1) == as.character(Player) & grepl("10 min", as.character(Detail)) == F),
                                      iPEND = 2*sum(as.character(Event) %in% c("PENL") & as.character(p2) == as.character(Player) & grepl("4 min", as.character(Detail)) == T) + sum(as.character(Event) %in% c("PENL") & as.character(p2) == as.character(Player) & grepl("10 min", as.character(Detail)) == F),
                                      DISTA = sum(na.omit(Distance*(as.character(Event) %in% c("SHOT", "GOAL", "MISS") & as.character(ev.team) == Away.Team)))),
                          group_by(pbp.full, Season, Date, Game.ID, Away.Goalie, Score.Cat, Strength.State, Season.Type) %>% rename(Player = Away.Goalie) %>%
                            summarise(Venue = "Away", Team = first(Away.Team), TOI = round(sum(as.numeric(as.character(Event.Length)))/60, 2),
                                      ...
                                      A1 = sum(as.character(Event) %in% c("GOAL") & as.character(p2) == as.character(Player)),
                                      A2 = sum(as.character(Event) %in% c("GOAL") & as.character(p3) == as.character(Player)),
                                      SOA = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("SHOT", "MISS", "GOAL") & as.character(ev.team) == Home.Team),
                                      SOG = sum(as.numeric(as.character(Period)) >= 5 & as.character(Event) %in% c("GOAL") & as.character(ev.team) == Home.Team),
                                      iPENT = 2*sum(as.character(Event) %in% c("PENL") & as.character(p1) == as.character(Player) & grepl("4 min", as.character(Detail)) == T) + sum(as.character(Event) %in% c("PENL") & as.character(p1) == as.character(Player) & grepl("10 min", as.character(Detail)) == F),
                                      iPEND = 2*sum(as.character(Event) %in% c("PENL") & as.character(p2) == as.character(Player) & grepl("4 min", as.character(Detail)) == T) + sum(as.character(Event) %in% c("PENL") & as.character(p2) == as.character(Player) & grepl("10 min", as.character(Detail)) == F),
                                      DISTA = sum(na.omit(Distance*(as.character(Event) %in% c("SHOT", "GOAL", "MISS") & as.character(ev.team) == Away.Team))))) %>%
...
player.bygame <- rbind_list(group_by(pbp.full, Season, Date, Game.ID, h1.num, Score.Cat, Strength.State, Season.Type) %>% rename(Player = h1.num) %>%
  ...
  summarise(Position = first(roster.full$Position[match(Player, roster.full$Full.Name)]), Venue = first(Venue), Team = first(Team), TOI = sum(TOI),
            ...
            iBLK = sum(iBLK), SOA = sum(SOA), SOG = sum(SOG),
            PENT = sum(PENT), PEND = sum(PEND), iPENT = sum(iPENT), iPEND = sum(iPEND),
            iDIST = sum(DIST)) %>% data.frame() %>%
